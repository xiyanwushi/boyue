...
<div class="bodywrapper">
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
			<?php if($_GET['status'] == 1): ?><h1 class="pagetitle">商品管理</h1>
            <span class="pagedesc">对商品进行添加、修改、删除等操作</span>
			<?php else: ?>
			 <h1 class="pagetitle">商品审核</h1>
            <span class="pagedesc">对商户上传的商品进行审核操作</span><?php endif; ?>
        ...
        <div id="contentwrapper" class="contentwrapper lineheight21">
			...
			<?php if($_GET['status'] == 1): ?><div class="tableoptions">                    
				<button class="radius3" onclick="location.href='<?php echo U('edit');?>'">添加商品</button>
			</div><!--tableoptions--><?php endif; ?>
			<div class="tableoptions">        
				<form method="post" id="form">
					所属标签:
					<select name="feature" default="<?php echo ($_GET['feature']); ?>" style="width:90px; min-width:0;">
						<option value="">全部</option>
						<?php if(is_array($_feature['config'])): foreach($_feature['config'] as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v['name']); ?></option><?php endforeach; endif; ?>
					</select>
					商户ID:
					<input type="text" name="user_id" value="<?php echo ($_GET['user_id']); ?>" class="smallinput" style="width:200px;" />
					产品名称
					<input type="text" name="title" value="<?php echo ($_GET['title']); ?>" class="smallinput" style="width:200px;" />
			...
			<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><?php if($vo['user_id'] > 0): echo ($vo['user_id']); else: ?>-(平台)<?php endif; ?></td>
						<td><?php echo ($vo["title"]); ?></td>
						<td><?php echo get_feature($vo['feature']);?></td>
						<td><?php echo ($vo["sort"]); ?></td>
						<td><?php echo ($vo["market_price"]); ?></td>
						<td><?php echo ($vo['price']+$vo['points']); ?></td>
						<td><?php if($vo['weight']): echo ($vo["weight"]); ?>Kg<?php else: ?>-<?php endif; ?></td>
						<td class="center">
							<?php if($_GET['status'] == 1): ?><a href="<?php echo U('edit', 'id='.$vo['id']);?>">修改</a> | 
							<?php else: ?>
							<a href="<?php echo U('audit', 'id='.$vo['id']);?>">审核</a> |<?php endif; ?>
