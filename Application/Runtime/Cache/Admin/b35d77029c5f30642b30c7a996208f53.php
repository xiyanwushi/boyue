...
<div class="bodywrapper">
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
            ...
            <span class="pagedesc">对角色进行增加，修改或者删除等操作</span>
        </div><!--pageheader-->
        <!--script type="text/javascript" src="/Public/js/jquery.min.js"></script-->
        <script type="text/javascript" src="/Public/js/public-jq.js"></script>
       
        <div id="contentwrapper" class="contentwrapper lineheight21">
			<div class="tableoptions">                    
				<button class="radius3" onclick="location.href='<?php echo U('edit');?>'">添加角色</button>
			</div><!--tableoptions-->
			<form action="<?php echo U('del');?>" method="post" id='productForm'>
				<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
					...
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php if($vo['status']): ?>启用<?php else: ?>禁用<?php endif; ?></td>
							<td class="center">
								<?php if($vo['status']): ?><a href="<?php echo U('update',array('id'=>$vo['id'],'status'=>0));?>">禁用</a> | 
								<?php else: ?>
									<a href="<?php echo U('update',array('id'=>$vo['id'],'status'=>1));?>">启用</a> |<?php endif; ?>
								<a href="<?php echo U('role', 'id='.$vo['id']);?>">配置权限</a> | 
								<a href="<?php echo U('edit', 'id='.$vo['id']);?>">修改</a> | 
								<a href="<?php echo U('del', 'id='.$vo['id']);?>" onclick="return confirm('你确实要删除这个角色吗？')">删除</a>
							</td>
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					<?php if(empty($list)): ?><tr>
						<td colspan="5">暂时没有商品</td>
					</tr><?php endif; ?>
					</tbody>
				</table>
			</form>
			<div class="dataTables_paginate paging_full_numbers" id="dyntable2_paginate"><?php echo ($page); ?></div>
