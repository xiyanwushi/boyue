        ...
        <div id="contentwrapper" class="contentwrapper lineheight21">
			...
			<div class="tableoptions">                    
				<form method="post">
					...
					返拥用户:
					<input type="text" name="self_id" value="<?php echo ($_GET['self_id']); ?>" placeholder="请输入用户编号" class="smallinput" style="width:100px;" />
					订单号:
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<?php echo get_separate_status($vo['status']);?>
							<!-- <?php if($vo['status'] == 0): ?>未达到条件
							<?php elseif($vo['status'] == 1): ?>待确认	
