...
<div class="bodywrapper">
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
            <h1 class="pagetitle">团队奖设置设置</h1>
            <span class="pagedesc">设置您的团队奖比例（按照分销奖比例金额的比例）</span>
        ...
        <div id="contentwrapper" class="contentwrapper lineheight21">
            ...
            <form class="stdform stdform2" method="post">
				<p id="set">
					<label>选择等级设置比例(单位：百分比)
						<small>
							<a href="javascript:;" class="addbtn" onclick="add();">添加等级比例</a>
							<a href="javascript:;" class="addbtn" onclick="del();">删除等级比例</a>
						</small>
					</label>
					<?php if(is_array($_tward)): foreach($_tward as $key=>$val): ?><span class="field">
						选择等级：
						<select name="level[]" style="width:150px;min-width:0px;" default="<?php echo ($key); ?>">
						<?php if(is_array($_level)): foreach($_level as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v['name']); ?></option><?php endforeach; endif; ?>
						</select>
						设置比例：
						<input type="text" name="rate[]" id="rate" value="<?php echo ($val); ?>" class="smallinput" style="width:150px;" />
			...
			<script>
				...
				function add(){
					html = '<span class="field">'
						+'选择等级：'
						+'<select name="level[]" style="width:150px;min-width:0px;">'
						+'<?php if(is_array($_level)): foreach($_level as $k=>$v): ?>'
						+'<option value="<?php echo ($k); ?>"><?php echo ($v['name']); ?></option>'
						+'<?php endforeach; endif; ?>'
						+'</select>'
						+'设置比例：'
						+'<input type="text" name="rate[]" id="rate" value="" class="smallinput" style="width:150px;" />'
						+'</span>'
					$('#set').append(html);
				}
				function del(){
					var ind = $('#set').find('span').length;
					$('#set').find('span').eq(ind-1).remove();
				}
			</script>
