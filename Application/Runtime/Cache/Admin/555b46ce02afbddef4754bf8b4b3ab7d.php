			...
			<table cellpadding="0" cellspacing="0" border="0" class="stdtable">
				...
				<tbody>
					...
					<?php if(is_array($product)): $i = 0; $__LIST__ = $product;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>
							<?php if($vo['status'] == -1): ?><a href="<?php echo U('Mch/cancleOne',array('cart_id'=>$vo['id'],'order_id'=>$info['id']));?>">同意退货</a>
								|
								<a href="<?php echo U('Mch/cancleOne',array('cart_id'=>$vo['id'],'order_id'=>$info['id'],'jujue'=>1));?>">拒绝退货</a>
