            ...
            <form class="stdform stdform2" method="post">
				...
				<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
                    ...
                    <tbody>
                    	...
                    	<?php if(is_array($_feature['config'])): $i = 0; $__LIST__ = $_feature['config'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td>
								<iframe src="<?php echo U('upload?cur_index='.$i.'&event=setPath&url='.$vo['pic']);?>" scrolling="no" style="width:200px; height:100px;"></iframe>
								<input type="hidden" name="pic[]" value="<?php echo ($vo["pic"]); ?>" />
							</td>
                            <td style="text-align:center"><input type="text" style="text-align:center" name="name[]" value="<?php echo ($vo["name"]); ?>" /></td>
							<td style="text-align:center">
								<label><input type="checkbox" <?php if($vo['checked'] == 1): ?>checked<?php endif; ?> name="checked<?php echo ($i); ?>" value="1" />是否积分购买</label>
							</td>
							<td style="text-align:center">
								<label><input type="radio" <?php if($vo['show'] == 1): ?>checked<?php endif; ?> name="show<?php echo ($i); ?>" value="1" />只显示图片</label>
								<label><input type="radio" <?php if($vo['show'] == 2): ?>checked<?php endif; ?> name="show<?php echo ($i); ?>" value="2" />只显示名称</label>
								<label><input type="radio" <?php if($vo['show'] == 3): ?>checked<?php endif; ?> name="show<?php echo ($i); ?>" value="3" />图片名称都显示</label>
							</td>
				...
				function add_row(){
					...
					html = '<tr>'
						  +'	 <td>'
						  +'		<iframe src="<?php echo U('upload?cur_index=_n_&event=setPath');?>" scrolling="no" style="width:200px; height:100px;"></iframe>'
